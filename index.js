...
var store = (module.exports.store = function(opts) {
  ...
  if (!opts.format) {
    log.error(
      'Please provide a --format param. Options are json, csv, csvs, jsonp, mongodb'
    );
    return;
  }
  ...
  mkdirp('flat/individuals', function() {
    ...
    if (opts.format === 'csv' || opts.format === 'csvs') {
      ...
      data.forEach(function(d) {
        if (opts.format == 'jsonp') {
          fs.writeFileSync(
            './flat/individuals/' + d._id + '.json',
            opts.callback + "('" + JSON.stringify(d) + "');"
          );
        } else {
          fs.writeFileSync(
            './flat/individuals/' + d._id + '.json',
      ...
      fs.writeFileSync(
        './flat/roster_short.json',
        JSON.stringify(roster_short)
      );
    }
  });
}
...
var phonemes = function(data, opts) {
  ...
  phonemes = helper.entries(phonemes).map(function(d) {
    return {
      ...
      percents: helper.entries(d.value.percents).filter(function(d) {
        return d.value != 0;
      })
    };
  });
